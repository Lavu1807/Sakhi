...
const LOW_ENERGY_PROTEIN_FOODS = [
  "eggs",
  "greek yogurt",
  "paneer",
  "lentils",
...
const ACTIVE_LIFESTYLE_PROTEIN_FOODS = [
  "chicken breast",
  "chickpeas",
  "salmon",
  "cottage cheese",
...
function resolvePhaseKey(phase) {
  ...
  return PHASE_ALIAS_MAP[normalized] || phase.trim();
}
...
export function filterAllergies(foods = [], allergies = []) {
...
export function personalizeFoods(foods, user = {}) {
...
export function getNutrition(phase, symptoms = []) {
...
export function getSymptomOptionsForPhase(phase) {
  const phaseKey = resolvePhaseKey(phase);
  const phaseData = nutritionData?.phases?.[phaseKey];

  if (!phaseData?.symptoms) {
    return [];
  }

  return Object.keys(phaseData.symptoms).map((symptomKey) => ({
    key: symptomKey,
    label: formatLabel(symptomKey),
  }));
}

export function getSmartNutrition(phase, symptoms = [], user = {}) {
